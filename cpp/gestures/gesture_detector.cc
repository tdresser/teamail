...
enum TimeoutEvent { SHOW_PRESS = 0, LONG_PRESS, TAP, TIMEOUT_EVENT_COUNT };
...
class GestureDetector::TimeoutGestureHandler {
 ...
 private:
  ...
  // OneShotTimer timeout_timers_[TIMEOUT_EVENT_COUNT];
  ReceiverMethod timeout_callbacks_[TIMEOUT_EVENT_COUNT];
  float timeout_delays_[TIMEOUT_EVENT_COUNT];
};
    ...
    case MotionEvent::Action::DOWN: {
      ...
      if (double_tap_listener_ && should_process_double_tap) {
        ...
        if (is_repeated_tap && had_tap_message) {
          ...
          // This is a first tap.
          DCHECK(double_tap_timeout_ > float());
...
void GestureDetector::Init(const Config& config) {
  ...
  const float maximum_swipe_deviation_angle =
      ClampToRange(config.maximum_swipe_deviation_angle, 0.001f, 45.0f);
...
bool GestureDetector::IsRepeatedTap(const MotionEvent& first_down,
  ...
  const float delta_time = second_down.GetEventTime() - first_up.GetEventTime();
